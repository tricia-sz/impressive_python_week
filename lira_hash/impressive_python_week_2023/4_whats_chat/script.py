...
# O campo e botao de enviar mensagem
...
def main(page):
    ...
    def enviar_mensagem_tunel(infos):
        ...
        if infos["tipo"] == "entrada":
            chat.controls.append(ft.Text(
                f"{usuario} entrou no chat", size=12, color=ft.colors.AMBER_400, italic=True))
        else:
    ...
    def enviar_mensagem(e):
        page.pubsub.send_all({"usuario": nome_usuario.value,
                             "mensagem": campo_mensagem.value, "tipo": "mensagem"})
